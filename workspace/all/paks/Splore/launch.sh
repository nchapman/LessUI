...
if [ ! -d ./pico-8 ]; then
	...
	if [ -n "$PICO8_ZIP" ]; then
		$PRESENTER "Extracting PICO-8..." 60 &
		PRESENTER_PID=$!

		if ! unzip -o "$PICO8_ZIP" -d ./ >> ./log.txt 2>&1; then
			kill "$PRESENTER_PID" 2>/dev/null
			$PRESENTER "Failed to extract PICO-8.\n\nCheck log.txt for details." 4
			exit 1
		fi
		...
		$PRESENTER "PICO-8 zip file not found!\n\nPlace pico-8*raspi.zip in:\n$DIR" 4
		exit 1
	fi
fi
...
if [ ! -d ./pico-8 ]; then
	$PRESENTER "PICO-8 directory missing!\n\nExtraction may have failed." 4
	exit 1
fi

if [ ! -f ./pico-8/pico8_64 ]; then
	$PRESENTER "PICO-8 binary not found!\n\nCheck pico-8 directory." 4
	exit 1
fi
